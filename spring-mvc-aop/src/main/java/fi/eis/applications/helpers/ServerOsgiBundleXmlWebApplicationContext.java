...
// http://grepcode.com/file_/repo1.maven.org/maven2/com.tqlab.sense/tqlab-sense-mvc/1.0.0/com/tqlab/sense/context/ServerOsgiBundleXmlWebApplicationContext.java/?v=source
public class ServerOsgiBundleXmlWebApplicationContext extends
	...
	public ServerOsgiBundleXmlWebApplicationContext(String[] configLocations) {
		...
		setDisplayName("Root ServerOsgiBundleXmlWebApplicationContext");
		logger.debug(
				"Creating an ServerOsgiBundleXmlWebApplicationContext with locations [{}].",
	...
	public ServerOsgiBundleXmlWebApplicationContext(ApplicationContext parent) {
		...
		setDisplayName("Root ServerOsgiBundleXmlWebApplicationContext");
		logger.debug(
				"Creating an ServerOsgiBundleXmlWebApplicationContext with parent [{}].",
				parent);
	}
		...
		if (bundleContext == null) {
			...
			if (this.servletContext != null) {
				...
				if (bundleContextFromServletContext != null) {
					Assert.isInstanceOf(BundleContext.class,
							bundleContextFromServletContext);
					this.logger
							.debug("Using the BundleContext stored in the ServletContext as '{}'.",
			...
			if (bundleContext == null) {
				...
				if (parent instanceof ConfigurableOsgiBundleApplicationContext) {
					this.logger
							.debug("Using the parent ApplicationContext's BundleContext");
	...
	public void setServletConfig(ServletConfig servletConfig) {
		...
		if (servletConfig != null) {
			...
			if (getNamespace() == null) {
				setNamespace(servletConfig.getServletName()
	...
	public void setNamespace(String namespace) {
		...
		if (namespace != null) {
			setDisplayName("ServerOsgiBundleXmlWebApplicationContext for namespace '"
					+ namespace + "'");
		}
	}
	...
	protected void postProcessBeanFactory(
		...
		if (bundleContext != null) {
			ServiceReference sr = bundleContext
					.getServiceReference(OsgiBeanFactoryPostProcessor.class.getName());
			if (sr != null) {
				OsgiBeanFactoryPostProcessor kernelPostProcessor =
						(OsgiBeanFactoryPostProcessor) bundleContext
						.getService(sr);
				try {
					kernelPostProcessor.postProcessBeanFactory(bundleContext,
							beanFactory);
				} catch (Exception e) {
					throw new ApplicationContextException(
							"Kernel bean factory post processor failed", e);
				} finally {
	...
	public Resource getResource(String location) {
		if (hasNoPrefix(location)) {
			return this.servletContextResourcePatternResolver
